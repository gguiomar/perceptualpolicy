...
class ActiveAvoidanceEnv2D:
    ...
    State: [norm_x, norm_y, tone_on, norm_time_since_tone]
    ...
    # Task definitions
    TASK_X_SHUTTLE = 1 # Requires crossing the vertical midline
    TASK_Y_SHUTTLE = 2 # Requires crossing the horizontal midline

    def __init__(self, grid_size=10, tone_duration_steps=50,
                 shock_delay_steps=50, max_steps_per_episode=100,
                 initial_task=TASK_X_SHUTTLE):
        """
        ...
        Args:
            ...
            initial_task: Task the environment starts with (TASK_X_SHUTTLE or TASK_Y_SHUTTLE).
        """
        self.grid_size = grid_size
        # Calculate center coordinates for midline checks
        self.center_x = (grid_size - 1) / 2.0
        self.center_y = (grid_size - 1) / 2.0
        ...
        # Define observation space (4 continuous values, normalized)
        self.observation_space = BoxSpace(
            low=np.array([0.0, 0.0, 0.0, 0.0]),
            high=np.array([1.0, 1.0, 1.0, 1.0]),
            shape=(4,), # State is [norm_x, norm_y, tone_on, norm_time]
            dtype=np.float32
        )

        # Internal variables tracking the current state of the environment
        self.agent_pos = [0, 0] # Agent's [x, y] coordinates
        self.tone_on = 0.0      # Flag indicating if the tone cue is active
    ...
    def _get_state(self):
        ...
        norm_x = self.agent_pos[0] / (self.grid_size - 1)
        norm_y = self.agent_pos[1] / (self.grid_size - 1)
        # Normalize time elapsed since tone started
        norm_time = min(self.time_since_tone_onset / self.tone_duration_steps, 1.0)
        # Return the state vector
        return np.array([norm_x, norm_y, self.tone_on, norm_time], dtype=np.float32)
    ...
    def reset(self):
        ...
        self.tone_on = 1.0 # As implemented right now, the tone is always on at the start of a trial
        ...
        start_x, start_y = 0, 0
        if self.current_task_id == self.TASK_X_SHUTTLE: # Task 1: Need to cross vertical midline
            if np.random.rand() < 0.5: # Start left half
                start_x = np.random.randint(0, int(np.floor(self.center_x)) + 1)
            else: # Start right half
                start_x = np.random.randint(int(np.ceil(self.center_x)), self.grid_size)
            start_y = np.random.randint(0, self.grid_size)
        elif self.current_task_id == self.TASK_Y_SHUTTLE: # Task 2: Need to cross horizontal midline
            if np.random.rand() < 0.5: # Start bottom half
                start_y = np.random.randint(0, int(np.floor(self.center_y)) + 1)
            else: # Start top half
                start_y = np.random.randint(int(np.ceil(self.center_y)), self.grid_size)
    ...
    def switch_task(self, new_task_id=None):
        ...
        if new_task_id: # Allow setting a specific task
            if new_task_id not in [self.TASK_X_SHUTTLE, self.TASK_Y_SHUTTLE]:
                raise ValueError("Invalid task ID")
            self.current_task_id = new_task_id
        else: # Default: toggle between Task 1 and Task 2
            self.current_task_id = self.TASK_Y_SHUTTLE if self.current_task_id == self.TASK_X_SHUTTLE else self.TASK_X_SHUTTLE
    ...
    def step(self, action):
        ...
        info = {'shocked': False, 'avoided': False, 'task': self.current_task_id} # Info about outcome
        ...
        next_pos[0] = np.clip(next_pos[0], 0, self.grid_size - 1)
        next_pos[1] = np.clip(next_pos[1], 0, self.grid_size - 1)

        # Update the agent's position
        self.agent_pos = next_pos

        # --- Main trial logic: applies only if the tone cue is active ---
        if self.tone_on > 0.5:
            ...
            avoided = False
            if self.current_task_id == self.TASK_X_SHUTTLE: # Task 1 rule
                if self.current_zone[0] != new_zone[0]: # Crossed vertical midline
                    avoided = True
            elif self.current_task_id == self.TASK_Y_SHUTTLE: # Task 2 rule
                if self.current_zone[1] != new_zone[1]: # Crossed horizontal midline
                    avoided = True
            ...
            if avoided:
                ...
                info['avoided'] = True
                self.tone_on = 0.0 # Turn off the tone
            else:
    ...
    def render(self, ax=None):
        ...
        if self.current_task_id == self.TASK_X_SHUTTLE:
