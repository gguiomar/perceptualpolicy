...
class GridWorld:
    ...
    def __init__(self, grid_size=10, start=(0, 0), goal=(9, 9), max_steps=50, 
    ...
    def step(self, action):
        ...
        if tuple(self.agent_pos) == self.goal:
            ...
            if new_distance > 0 and new_distance < 1:
                reward = 0.15  # central bonus
            elif new_distance >= 1 and new_distance < 2:
                reward = 0.10
            elif new_distance >= 2 and new_distance < 3:
                reward = 0.05
            elif new_distance >= 3 and new_distance < 4:
                reward = -0.00
            elif new_distance >= 4 and new_distance < 5:
                reward = -0.05  # central bonus
            elif new_distance >= 5 and new_distance < 6:
                reward = -0.10
            elif new_distance >= 6 and new_distance < 7:
                reward = -0.15
            elif new_distance >= 7 and new_distance < 8:
                reward = -0.20
            elif new_distance >= 8:
                reward = -0.25
            done = False
